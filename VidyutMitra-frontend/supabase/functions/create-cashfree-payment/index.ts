...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (paymentType === "booking_advance") {
      ...
      orderNote = `Vidyut Mitra ₹21 booking token for ${user.id}`;
      ...
      orderNote = `Vidyut Mitra final payment for ${orderId}`;
    ...
    const cfBody: Record<string, unknown> = {
      ...
      customer_details: {
        customer_id: user.id,
        customer_email: user.email || "customer@vidyutmitra.local",
