...
import {
  Zap, PlugZap, MapPin, ShoppingCart, User, LogOut, ClipboardList,
...
function Header({ user, profile, role, onLogout, onLogin, onSignup }) {
  return <header className="topbar">
    <div className="brand"><span className="brandIcon"><PlugZap size={21}/></span><span>Vidyut Mitra</span></div>
...
function Auth({ onDone,onBackHome ,initialMode = "login" }) {
  ...
  return <div className="authPage">
    <div className="authCard">
      <div className="authLogo"><PlugZap size={25}/></div>
      <h1>{mode==="login" ? "Welcome back" : "Create your Vidyut Mitra account"}</h1>
...
function App() {
 ...
 useEffect(() => {
  ...
  if(loading) return <div className="loading"><PlugZap/> Loading Vidyut Mitra…</div>;
